...
const API_LINK = "http://localhost:7040/";
...
function Main() {
    ...
    const handleSubmitByGraph = data => {
        axios.post(API_LINK + "api/attempts/try", data).then(response => {
    ...
    useEffect(() => {
        ...
        const foo = () => {
            axios.get(API_LINK + "api/attempts/get").then(response => {
        ...
        foo();
        let intervalId = setInterval(foo, 5000);
        return () => {
...
const handleSubmit = () => {
    ...
    axios.post(API_LINK + "api/attempts/try", toBeSent).then(
        response => {
            setAttempts([...attempts, ...response.data]);
        }
    ).catch(err => {
                    ...
                    <table id='tb-result'>
                        <thead>
                            <tr>
                                ...
                                <th className='resth'>Result</th>
                                <th className='coordth'>X</th>
                                <th className='coordth'>Y</th>
                                <th className='rth'>R</th>
                                <th className='exth'>Execution time</th>
                                <th className='curtimeth'>Current time</th>
                            </tr>
                        </thead>
                        <tbody>
                            {
